...
class Maze():
    def __init__(self, size_pixels, cells_per_side, walls_width):
        ...
        self.grid = []
        self.maze_sur = Surface((size_pixels, size_pixels))
    ...
    def draw_walls(self, idx, walls:set=None):
        ...
        for neighbour in possible_neighbours:
            ...
            if not self._is_inside(Vector2(x, y) + neighbour) or (neighbour not in cell.connected_neighbours and (min(idx, neighbour_idx), max(idx, neighbour_idx)) not in walls): 
                #North
                if neighbour == Vector2(0,-1):
                    draw.line(self.maze_sur, "black", cell_pixelpos, cell_pixelpos + Vector2(self.cell_size, 0), width= self.walls_width)
                #South
                if neighbour == Vector2(0, 1):
                    draw.line(self.maze_sur, "black", cell_pixelpos + Vector2(0, self.cell_size), cell_pixelpos + Vector2(self.cell_size, self.cell_size), width= self.walls_width)
                #East
                if neighbour == Vector2(1, 0):
                    draw.line(self.maze_sur, "black", cell_pixelpos + Vector2(self.cell_size, 0), cell_pixelpos + Vector2(self.cell_size, self.cell_size), width= self.walls_width)
                #West
                if neighbour == Vector2(-1, 0):
                    draw.line(self.maze_sur, "black", cell_pixelpos, cell_pixelpos + Vector2(0, self.cell_size), width= self.walls_width)
                #Add this wall to the set
                walls.add((min(idx, neighbour_idx), max(idx, neighbour_idx)))
        

    def draw_maze(self, screen: Surface):
        self.maze_sur.fill("white")
    ...
    def color_cell(self, cell: Cell, color, screen: Surface):
        x = cell.position.x * self.cell_size
        y = cell.position.y * self.cell_size
        draw.rect(self.maze_sur, color, Rect(x, y, self.cell_size, self.cell_size))
        idx = int(cell.position.y * self.cells_per_side + cell.position.x)
        self.draw_walls(idx)
        blit_x = (screen.get_size()[0]-self.size_pixels)/2
        blit_y = (screen.get_size()[1]-self.size_pixels)/2
        screen.blit(self.maze_sur, (blit_x, blit_y))
